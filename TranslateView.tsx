...
interface TranslateViewProps {
  // FIX: Removed apiKey prop to align with new service implementation.
...
const TranslateView: React.FC<TranslateViewProps> = ({ 
  ...
  useEffect(() => {
    if (inputText.trim() === lastTranslatedItem?.fromText && sourceLang === lastTranslatedItem?.sourceLang) {
  ...
  const handleTranslate = useCallback(async () => {
    ...
    // FIX: Removed apiKey from translateText call.
    const result = await translateText(textToTranslate, sourceLangName, targetLangName);

    if (result && result.meanings.length > 0) {
      const primaryTranslation = result.meanings[0].translations[0] || 'N/A';
      
      setIsAudioLoading(true);
      // FIX: Removed apiKey from getTextToSpeech call.
      const audioBase64 = await getTextToSpeech(textToTranslate, sourceLang);
  ...
  }, [inputText, sourceLang, targetLang, history, addTranslationToHistory, showToast, sourceLangName, targetLangName, autoPlayAudio]);
